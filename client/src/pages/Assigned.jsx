...
export default function Assigned() {
    ...
    useEffect( ()=> {
        dispatch ({ type: VIEW, payload: "assigned"})
    },[state.view])
    ...
    // Tasks
    // Filter for Active Tasks for Unassigned
    const filterTasks = allTaskData.data.tasks.filter( (task) => !task.complete_flag && task.assigned._id !== "65c5681fada9c3ff849efeed") 
    // Sort by Review Date
    ...
    return (
        <div>
            <div className="brand text-3xl sm:text-3xl md:text-3xl lg:text-3xl xl:text-4xl">Assigned Tasks</div>    
